...
class DSLPIDControl(DroneEnvControl): # ドローンの位置および姿勢を制御するためのDSL-PIDコントローラクラス。
    ...
    def __init__(
        ...
        # これらの具体的な値は、特定のモーターやシステムの仕様に基づいて
        self._pwm2rpm_scale = 0.2685 # パルス幅変調（PWM）信号から回転数（RPM）に変換するためのスケール係数
        self._pwm2rpm_const = 4070.3 # パルス幅変調（PWM）信号から回転数（RPM）に変換するための定数項
        self._min_pwm = 20000
        self._max_pwm = 65535
    ...
    def coating_motion_forcefeedback_control_from_kinematics(
            self,
            control_timestep: float,
            kin_state: DroneKinematicsInfo,
            ctrl_target: DroneControlTarget,
        ...
        ctrl_target 制御対象の情報。
        """
        return self.coating_motion_forcefeedback_control(
            control_timestep=control_timestep,
            current_position=kin_state.pos,
            current_quaternion=kin_state.quat,
            current_velocity=kin_state.vel,
            ...
            target_rpy=ctrl_target.rpy,
            target_rpy_rates=ctrl_target.rpy_rates,
            target_force=ctrl_target.force,
            measured_force=np.ndarray,
        )

    def coating_motion_forcefeedback_control(
            self,
            control_timestep: float,
            current_position: np.ndarray,
            current_quaternion: np.ndarray,
        ...
        rpm = self.dsl_pid_coating_forcefeedback_attitude_control( # 姿勢制御関数の呼び出し
            control_timestep,
            thrust,
            current_quaternion,
            target_euler,
    ...
    def dsl_pid_coating_forcefeedback_attitude_control(
            self,
            control_timestep: float,
            thrust: float,
            current_quaternion: np.ndarray,
