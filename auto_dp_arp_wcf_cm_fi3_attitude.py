...
class DSLPIDControl(DroneEnvControl): # ドローンの位置および姿勢を制御するためのDSL-PIDコントローラクラス。
    ...
    def coating_attitude_control(
        ...
        gf_thrust = np.array([0, 0, self._gf]) # ホバリング
        # dot 行列積
...
if __name__ == "__main__":
    ...
    distance_slider = p.addUserDebugParameter("Camera Distance", -5, 10, 0.4)
    ...
    # 許容範囲内の位置差
    pos_tolerance = 0.1  # 適切な値に変更してください
    ...
    for i in range(step_num):
        ...
        if pos_difference[0] < pos_tolerance and orientation_difference[0] < orientation_tolerance and not flag_condition0:
        ...
        if pos_difference[1] < pos_tolerance and orientation_difference[1] < orientation_tolerance and not flag_condition1:
            print(f"目標位置 {current_target_index} に到達しました。")
            print(f"目標姿勢 {current_target_index} に到達しました。")
        ...
        if pos_difference[2] < pos_tolerance and orientation_difference[2] < orientation_tolerance and not previous_contacts and contacts and not flag_condition2:
            print(f"目標位置 {current_target_index} に到達しました。")
            print(f"目標姿勢 {current_target_index} に到達しました。")
