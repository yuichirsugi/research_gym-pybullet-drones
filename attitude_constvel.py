...
class DroneBltEnv(BulletEnv):
    ...
    def apply_rotor_physics(self, rpm: np.ndarray):
        ...
        # # 速度を与える
        # linear_velocity = [-0.5, 0, 0.5]  
        # p.resetBaseVelocity(self._drone_ids, linearVelocity=linear_velocity)
...
class DSLPIDControl(DroneEnvControl): # ドローンの位置および姿勢を制御するためのDSL-PIDコントローラクラス。
    ...
    def coating_attitude_control(
        ...
        gf_thrust = np.array([-0.03, 0, self._gf]) # ホバリング
        # gf_thrust = np.array([-0.04, 0, self._gf]) # ホバリング
