...
class DroneBltEnv(BulletEnv):
    ...
    def __init__(
            ...
            sim_freq: int = 240, # init_value
            # sim_freq: int = 100, # pressure device　cycle
...
class DSLPIDControl(DroneEnvControl): # ドローンの位置および姿勢を制御するためのDSL-PIDコントローラクラス。
    ...
    def __init__(
        ...
        self._integral_vel_e = np.zeros(3)
    ...
    def dsl_pid_position_control(
        ...
        target_thrust = np.multiply(self._PID.P_pos_for, pos_e) \
                        + np.multiply(self._PID.I_pos_for, self._integral_pos_e) \
                        + np.multiply(self._PID.D_pos_for, vel_e) \
    ...
    def coating_motion_control(
            ...
            target_rpy: np.ndarray,
            target_rpy_rates: np.ndarray = np.zeros(3),
            prev_velocity: np.ndarray = np.zeros(3),
    ) -> Tuple:

         # 速度制御
        thrust, computed_target_rpy = self.dsl_pid_coating_velocity_control(
            control_timestep,
            current_position,
            current_quaternion,
            current_velocity,
    ...
    def dsl_pid_coating_velocity_control(
        ...
        prev_velocity: np.ndarray,
            ...
            # print("Previous Velocity:", prev_velocity)
            # 現在の加速度を推定
            ...
            self._integral_vel_e = np.clip(self._integral_vel_e, -2., 2.)  # np.clip(array, min_value, max_value)
            # self._integral_vel_e = np.clip(self._integral_vel_e, -0.02, 0.02)
            self._integral_vel_e[2] = np.clip(self._integral_vel_e[2], -0.015, 0.015)
            target_thrust = np.multiply(self._PID.P_vel_for, vel_e) \
                            + np.array([-0.06, 0, self._gf]) \
                            # + np.multiply(self._PID.I_for, self._integral_vel_e) \
                            # + np.array([-0.0981, 0, self._gf])
...
if __name__ == "__main__":
    ...
    # ユーザーが調整可能なパラメータを追加
    yaw_slider = p.addUserDebugParameter("Yaw", -180, 180, 7) # 7
    pitch_slider = p.addUserDebugParameter("Pitch", -90, 90, -20)
    distance_slider = p.addUserDebugParameter("Camera Distance", -5, 10, 0.4) # 0.4 0.368
    target_x_slider = p.addUserDebugParameter("Camera TargetPos X", -3, 3, -0.4) # -0.4
    target_y_slider = p.addUserDebugParameter("Camera TargetPos Y", -3, 3, 0.37)
    target_z_slider = p.addUserDebugParameter("Camera TargetPos Z", 0, 3, 0.695) # 0.695
    # p.addUserDebugParameter(name=, rangeMin, rangeMax, startValue)
    ...
    step_num = 8_000
    # step_num = 2_000
    # log_interval = 1  # 姿勢を崩す場合→動きが遅すぎるから
    log_interval = 20 
    # log_interval = 100
    ...
    # target_vel = np.array([-0.01, 0, 0])  # 初期位置とその次の位置を設定
    target_vel = [
        [0, 0, 0],
        [0, 0, -0.1],
        ]  # 初期位置とその次の位置を設定
    ...
    for i in range(step_num):
        ...
        if i % log_interval == 0: # i が log_interval の倍数のときに条件が真になり、そのときにログが出力
            ...
            # print(f"Step {i}:")
            # print("Roll, Pitch, Yaw:", kis.rpy) #実測値
            ...
            if contacts:
                ...
                for contact_info in contacts:
                    ...
                    print(f" {i}, {kis.rpy[1]}, {contact_info[9]}") # 接触点での法線方向（垂直方向）への力
                    # 法線方向の力がプラスの場合、物体同士が互いに押し合っていることを示す
                    # brush_force = contact_info[9]
                    # print(f"  brush_forcex: {brush_force[0]}")
                    # print(f"  brush_forcey: {brush_force[1]}")
                    # print(f"  brush_forcez: {brush_force[2]}")
                    contact_position = tuple(contact_info[5])  # 接触点の位置をタプルに変換
                    p.addUserDebugLine(contact_position, (contact_position[0], contact_position[1], contact_position[2] + 0.01), [1, 0, 0], 3)  # 非常に短い線分を描画
                    # p.addUserDebugText("Contact Point", contact_position, textColorRGB=[1, 0, 0])  # テキストを追加
    
                    brush_force = contact_info[9]
                    target_force = 0.06

                    fa = 1.0 * brush_force # 動摩擦力 : 1.0
                    fb = abs(brush_force * np.tan(kis.rpy[1])) # abs():絶対値 マイナス姿勢角を考慮
                    horizontal_reaction_force = fa + fb
                    # print(f" fa:{fa}, fb:{fb}, hrf:{horizontal_reaction_force}")

                    angle_rad_0 = math.atan(target_force / (m * g - horizontal_reaction_force))
                    # T = 0.5 / np.linalg.norm(target_vel[1])
                    vector_scalar = np.linalg.norm(target_vel[1])
                    angle_rad_1 = math.atan(target_force / (m * g - horizontal_reaction_force - (m * pow(vector_scalar, 2))/ 0.5 ))
                    # print(f" angle_rad_0:{angle_rad_0}, angle_rad_1:{angle_rad_1}")
                    # print(f" Actual speed:{kis.vel}")

                    # アークサインを計算
                    angle_rad = math.asin(target_force / (m * g))
