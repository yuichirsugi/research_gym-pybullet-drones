...
class DSLPIDControl(DroneEnvControl): # ドローンの位置および姿勢を制御するためのDSL-PIDコントローラクラス。
    ...
    def coating_approaching_control_from_kinematics(
            self,
            control_timestep: float,
            kin_state: DroneKinematicsInfo,
            ctrl_target: DroneControlTarget,
        ...
        ctrl_target 制御対象の情報。
        """
        return self.coating_approaching_control(
            control_timestep=control_timestep,
            current_position=kin_state.pos,
            current_quaternion=kin_state.quat,
            current_velocity=kin_state.vel,
    ...
    def coating_approaching_control(
            self,
            control_timestep: float,
            current_position: np.ndarray,
            current_quaternion: np.ndarray,
         ...
         # 速度制御
        thrust, computed_target_rpy = self.dsl_pid_approaching_velocity_control(
            control_timestep,
            current_position,
            current_quaternion,
            current_velocity,
    ...
    def dsl_pid_approaching_velocity_control(
        self,
        control_timestep: float,
        current_position: np.ndarray,
        current_quaternion: np.ndarray,
        ...
        prev_velocity: np.ndarray,
            ...
            target_thrust = np.multiply(self._PID.P_vel_for, vel_e) \
                            ...
                            # + np.multiply(self._PID.I_vel_for, self._integral_vel_e) \
                            # + np.multiply(self._PID.D_vel_for, acc_e) \
                            # + np.array([0, 0, self._gf])
    ...
    def dsl_pid_coating_attitude_control(
        ...
        self._integral_rpy_e = np.clip(self._integral_rpy_e, -1500., 1500.)
        # self._integral_rpy_e[0:2] = np.clip(self._integral_rpy_e[0:2], -1., 1.) # _integral_rpy_e の最初の2つの要素（角速度エラーのxとy成分に対応）
        self._integral_rpy_e[0:2] = np.clip(self._integral_rpy_e[0:2], -0.5, 0.5) # _integral_rpy_e の最初の2つの要素（角速度エラーのxとy成分に対応）
        # PID target torques # PID制御で求めた目標トルク
        target_torques = - np.multiply(self._PID.P_tor, rot_e) \
                         + np.multiply(self._PID.D_tor, rpy_rates_e) \
                         + np.multiply(self._PID.I_tor, self._integral_rpy_e)
...
if __name__ == "__main__":
    ...
    target_vel2 = np.array([0, 0, -0.001])
    ...
    for i in range(step_num):
        ...
        if pos_difference[current_target_index] < pos_tolerance and not flag_condition0: # flag_condition0 が Falseならば
        ...
        if pos_difference[current_target_index] < pos_tolerance and not flag_condition1:
            ...
            if current_target_index < len(target_positions):
                current_target_vel = target_vel[current_target_index]

            # フラグをセット
            flag_condition1 = True

        if contacts: # flag_condition0がTrue, flag_condition2がFalse
             print("棒と壁が接触しています")
        ...
        if flag_condition0 and flag_condition1 and not flag_condition2: # flag_condition0がTrue, flag_condition2がFalse
