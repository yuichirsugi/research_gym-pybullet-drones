...
class DroneBltEnv(BulletEnv):
    ...
    def __init__(
        ...
        # print("self._init_xyzs:", self._init_xyzs)

        # # ドローンの作成
        # drone_id = p.loadURDF(self._urdf_path)
        ...
        # # 衝突判定有効化
        # p.setCollisionFilterPair(self._drone_ids, wall_body, -1, -1, enableCollision=True, physicsClientId=self._client)

        # p.setCollisionFilterPair(
        #     bodyUniqueIdA,  # 衝突フィルターを設定する物体Aの識別子
        #     bodyUniqueIdB,  # 衝突フィルターを設定する物体Bの識別子
        #     linkIndexA,     # 物体Aのリンクインデックス（-1の場合はすべてのリンク）
        #     linkIndexB,     # 物体Bのリンクインデックス（-1の場合はすべてのリンク）
        #     enableCollision,  # 衝突を有効にする場合はTrue、無効にする場合はFalse
        #     physicsClientId  # 使用する物理エンジンのクライアントID
        # )
        ...
        print("self._drone_ids:", self._drone_ids)

        self.target_link_name = "brush_link"

        # ロボット内のジョイント数を取得
        self.num_joints = p.getNumJoints(self._drone_ids)
    ...
    def get_drone_inf(self) : # 新しく定義
        return self._drone_ids, self.target_link_name, self.num_joints, self.wall_id
...
if __name__ == "__main__":
    ...
    distance_slider = p.addUserDebugParameter("Camera Distance", -5, 10, 0.3)
    ...
    # 対象のリンクに関する情報を取得
    target_joint_index = -1  # 初期値

    drone_id, target_link_name, num_joints, wall_id = env.get_drone_inf()
    ...
    for joint_index in range(num_joints):
        joint_info = p.getJointInfo(drone_id, joint_index)
    ...
    if target_joint_index != -1:
        ...
        for i in range(step_num):
            ...
            # 対象のリンクにかかる力とトルクを取得
            link_state = p.getLinkState(drone_id, target_joint_index)
            force_on_link = link_state[2]  # 2は力を表すインデックス
            torque_on_link = link_state[3]  # 3はトルクを表すインデックス
