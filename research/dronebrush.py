...
if __name__ == "__main__":
    ...
    # rpms = np.array([10000, 10000, 10000, 10000])
    rpms = np.array([7000, 7000, 7000, 7000])
    ...
    # target_speed = 0.01  # ゆっくりと進む目標速度（任意の値に調整）
    ...
    for i in range(step_num):
        ...
        # 現在の目標位置
        current_target = target_positions[target_index]

        # 目標速度を計算し正規化
        # current_velocity = target_speed * (current_target - kis[0].pos) / np.linalg.norm(current_target - kis[0].pos) # 単位ベクトルを使用
        ...
        rpms, _, _ = ctrl.compute_control_from_kinematics( # drone_ctrl.pyのメソッド
            ...
            ctrl_target=DroneControlTarget( # ctrl_target: 制御の目標となる値。DroneControlTargetクラスのインスタンスで、目標位置が指定されている
                pos=current_target, # DroneControlTarget : data_difinition.py 
                # vel=current_velocity, # ここで目標速度を設定
                # vel=target_velocity - damping_term  # 減衰項を適用
        ...
        # 目標位置に到達したら次の目標位置に移動
        if np.linalg.norm(kis[0].pos - current_target) < 0.2: # np.linalg.norm:ベクトルの大きさ
            target_index = (target_index + 1) % len(target_positions) # len(target_positions) は target_positions リストの要素数
            # 例 2回目の目標位置に到達後 → target_index は (1 + 1) % 3 と計算され、結果は 2
